...
class CreateProductIntegrationTest() {
    ...
    fun `createProduct returns 401 on Invalid request`() {
        ...
        val invalidRequest =
            CreateProductRequest(
                ...
                category =
                    "INVALID_CATEGORY_NAME_EXCEEDING_LENGTH_LIMIT_" +
                        "Y".repeat(100) // exceeds @Size(max=64) and not a valid enum
                )
    ...
    fun `getProductById returns 200 with a single item`() {
        ...
        assertNotNull(responseBody.id)
        assertEquals(
            createdProduct.createdAt.toEpochMilli(), responseBody.createdAt?.toEpochMilli())
    ...
    fun `getProductById returns 404 when invalid is used`() {
        ...
        webTestClient
            ...
            .isEqualTo("Not Found")
            .jsonPath("$.path")
            .isEqualTo("/api/v1/products/$missingId")
    }
    ...
    fun `updateProductById returns 200 with product when passed empty UpdatedProductRequest`() {
        ...
        val req =
            createReq(
                ...
                price = BigDecimal("9.99"))
        val updateProductRequest =
            UpdateProductRequest(name = null, description = null, price = null, category = null)
    ...
    fun `updateProductById return 400 when invalid req is passed`() {
        ...
        val updateProductRequest =
            UpdateProductRequest(
                ...
                category =
                    "INVALID_CATEGORY_NAME_EXCEEDING_LENGTH_LIMIT_" +
                        "Y".repeat(100) // exceeds @Size(max=64)
                )
        ...
        val updatedProduct =
            webTestClient
                ...
                .isEqualTo("Bad Request")
                .jsonPath("$.path")
                .isEqualTo("/api/v1/products/${createdProduct.id}")
    }
    ...
    fun `deleteProductById return 204 on successful deletion`() {
        ...
        // then
        webTestClient.get().uri(uri + "/${createdProduct.id}").exchange().expectStatus().isNotFound
    }
    ...
    private fun createReq(
        ...
        CreateProductRequest(
            sku = sku, name = name, description = description, price = price, category = category)
    ...
    private fun randomObjectId(): String = List(24) { "0123456789abcdef".random() }.joinToString("")
}
